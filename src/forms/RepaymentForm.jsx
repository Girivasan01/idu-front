 ...
 */
import { Form, Input, InputNumber, Select, DatePicker, Row, Col, Button, Tag, Grid } from 'antd';
...
export default function RepaymentForm({ isUpdateForm = false }) {
  ...
  const statusOptions = useMemo(() => {
    ...
    switch (normalizedOriginalStatus) {
      case 'not_started':
        return STATUS_OPTIONS.filter((o) => ['paid', 'partial', 'default'].includes(o.value));
      case 'default':
        return STATUS_OPTIONS.filter((o) => ['paid', 'partial'].includes(o.value));
      case 'partial':
        return STATUS_OPTIONS.filter((o) => ['paid'].includes(o.value));
      case 'paid':
        return STATUS_OPTIONS.filter((o) => o.value === 'paid');
      case 'late':
        return STATUS_OPTIONS.filter((o) => o.value === 'late');
      default:
  ...
  useEffect(() => {
    if (isUpdateForm && normalizedOriginalStatus === 'paid') form.setFieldValue('status', 'paid');
  ...
  useEffect(() => {
    // For update flow, when moving to Paid/Late and no payment date exists,
    // default to today while still allowing manual edits.
    if (!isUpdateForm) return;
    if (!['paid', 'late'].includes(normalizedStatus)) return;
    if (paymentDate) return;
    form.setFieldValue('paymentDate', dayjs());
  }, [form, isUpdateForm, normalizedStatus, paymentDate]);
  ...
  const handlePartialPayment = useCallback(() => {
    ...
    if (addPayment <= 0) {
      form.setFields([
        { name: 'additionalPayment', errors: ['Additional payment must be greater than 0'] },
      ]);
      return;
    }
    ...
    form.setFieldsValue({
      amountPaid: round2(paidAmount + addPayment),
  ...
  return (
    <>
      ...
      {isUpdateForm && normalizedStatus && (
        ...
        >
          ...
          <Tag
            ...
            style={{ borderRadius: 12, fontSize: 13, margin: 0 }}
          >
        ...
        {isUpdateForm ? (
          ...
          <div style={{ pointerEvents: 'none' }}>
            {clientDisplayName !== null ? (
              ...
              <Input value={clientDisplayName} readOnly style={ROStyle} />
            ) : (
      ...
      <Form.Item name="_originalStatus" hidden noStyle>
        <Input />
      </Form.Item>
      <Form.Item name="_originalAmountPaid" hidden noStyle>
        <Input />
      </Form.Item>
      ...
      {['paid', 'late'].includes(normalizedStatus) && (
        <>
          ...
          <Form.Item name="amountPaid" hidden noStyle>
            <InputNumber />
          </Form.Item>
        </>
      )}
