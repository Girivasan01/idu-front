...
const ClientMapLink = ({ client }) => {
  const mapLink = client?.map_link || client?.mapLink || '';
...
export default function ClientRepayment() {
  ...
  const formatRepaymentPayload = (values) => {
    const amountPaid =
      values.amountPaid !== undefined ? values.amountPaid : (currentRepayment?.amountPaid ?? 0);
  ...
  const fetchRepayments = () => {
    dispatch(
      crud.list({ entity: 'repayment', options: { filter: 'client', equal: id, items: 100 } })
    );
  };
  ...
  const handleEdit = (record) => {
    ...
    form.setFieldsValue({
      ...
      paymentDate: record.paidDate
        ? dayjs(record.paidDate)
        : record.paymentDate
          ? dayjs(record.paymentDate)
          : null,
  ...
  const handleDownloadBill = async (record) => {
    try {
      const auth = storePersist.get('auth');
      const token =
        auth?.token || auth?.current?.token || window.localStorage.getItem('token') || null;
      const response = await fetch(`${API_BASE_URL}/repayment/download/${record._id}`, {
        method: 'GET',
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });

      if (!response.ok) {
        let msg = 'Failed to download bill';
        try {
      ...
      link.href = blobUrl;
      link.download = `repayment-${record._id}.pdf`;
        ...
        return (
          ...
          >
            <Tag
              ...
              style={{
                cursor:
                  (allowedTransitions[displayStatus] || []).length > 0 ? 'pointer' : 'default',
              }}
            >
    ...
    },
    {
      ...
      render: (_, record) => (
        <div style={{ display: 'flex', gap: '10px' }}>
          <Tooltip
            title="Edit"
            overlayInnerStyle={{ fontSize: '11px', padding: '2px 8px' }}
          ...
          <Card loading={isClientLoading}>
            <Row gutter={[16, 16]} align="middle">
              <Col xs={24} md={3} style={{ display: 'flex', alignItems: 'center' }}>
                <CustomerAvatar photo={client?.photo} name={client?.name} size={72} />
              </Col>
              {[
                { label: translate('Client'), value: client?.name },
                {
                  label: translate('Loan Amount'),
                ...
                <Col xs={12} md={5} key={label}>
                  <small style={{ color: '#8c8c8c', fontSize: 12, display: 'block' }}>
                    {label}
                  </small>
                  <h3 style={{ margin: '4px 0 0', fontSize: 16, wordBreak: 'break-word' }}>
                    {value}
                  </h3>
                </Col>
              ))}
          ...
          <>
            ...
            >
              ...
              <Descriptions.Item label="Ending Date">
                {client.endDate ? dayjs(client.endDate).format(dateFormat) : '-'}
              </Descriptions.Item>
              ...
              <Descriptions.Item label="Status">
                <Tag
                  color={
                    client.status === 'active' ? 'blue' : client.status === 'paid' ? 'green' : 'red'
                  }
                >
              ...
              {(() => {
                ...
                if (bankName || accountNumber)
                  return (
                    <>
                      <Descriptions.Item label="Payment Mode">
                        <Tag color="blue">Bank Transfer</Tag>
                      </Descriptions.Item>
                      {bankName && (
