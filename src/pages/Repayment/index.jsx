...
const downloadRepaymentBill = async (repaymentId) => {
  const token = getAuthToken();
  const response = await fetch(`${API_BASE_URL}/repayment/download/${repaymentId}`, {
    method: 'GET',
  ...
  const fileName = fileNameMatch?.[1]?.replace(/"/g, '') || `repayment-${repaymentId}.pdf`;
...
export default function Repayment() {
  ...
  const openRepaymentEditor = useCallback((repayment) => {
    if (!repayment) return;
    const displayStatus = getDisplayStatus(repayment);
  ...
  const modalClient = useMemo(() => {
    const editingClient = editingRepayment?.client;
    const editingClientId = resolveClientId(editingClient);
    if (!editingClientId) return null;

    const fullClient = clients.find((item) => String(item?._id) === editingClientId) || null;

    if (editingClient && typeof editingClient === 'object') {
      return {
        ...fullClient,
        ...editingClient,
        photo: editingClient.photo || fullClient?.photo,
      };
    }

    return fullClient;
  }, [editingRepayment, clients]);
    ...
    <ErpLayout>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="ok" type="primary" loading={submitLoading} onClick={handleEditModalOk}>
            {translate('OK')}
          </Button>,
        ]}
        ...
        {modalClient ? (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
              <CustomerAvatar photo={modalClient?.photo} name={modalClient?.name} size={44} />
            ...
            {editingRepayment?._id ? (
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    await downloadRepaymentBill(editingRepayment._id);
                  } catch (error) {
                    notification.error({
