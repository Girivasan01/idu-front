/**
 * pages/customer/CustomerCalendar.jsx — DV Finance
            ...
            >
              ...
              {events.length > 0 && (
                <Badge
                  count={events.length}
                  style={{
                    backgroundColor: isToday ? '#fff' : BOX_BORDER,
                    color: isToday ? BOX_BORDER : '#fff',
                  }}
                />
              )}
            </div>
              ...
              <div style={{ padding: '8px 12px', background: '#fff' }}>
                {events.map((event) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        style={{ fontWeight: 600, fontSize: 14, color: calendarStyle.style.color }}
                      >
...
export default function CustomerCalendar() {
  ...
  const formatPayload = (values) => {
    const amountPaid =
      values.amountPaid !== undefined ? values.amountPaid : (editingRepayment?.amountPaid ?? 0);

    return {
      ...values,
      amountPaid:
        ['paid', 'late'].includes(values.status) && !amountPaid ? values.amount : amountPaid,
      date: values.date ? dayjs(values.date).toISOString() : undefined,
      paymentDate: values.paymentDate
        ? dayjs(values.paymentDate).toISOString()
        : ['paid', 'late'].includes(values.status)
  ...
  const handleEditOk = async () => {
    ...
    try {
      ...
      setRepayments((prev) =>
        prev.map((r) =>
          r._id === updateResponse.result._id ? { ...r, ...updateResponse.result } : r
        )
      );
        ...
        <PageHeader
          ...
          extra={[
            <Space
              ...
              style={{ width: '100%', alignItems: isMobile ? 'flex-end' : 'center' }}
            >
              <Button
            ...
            <Col xs={24} sm={8}>
              <Space>
                ...
                <div>
                  ...
                  <div style={{ fontWeight: 700, fontSize: isMobile ? 15 : 17 }}>
                    {client?.name || '—'}
                  </div>
                </div>
              </Space>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      {hasUPI && <Descriptions.Item label="UPI ID">{upiId}</Descriptions.Item>}
                    </>
                  );
                })()}
              </Descriptions>
