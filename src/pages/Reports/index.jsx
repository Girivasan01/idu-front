/**
 * pages/Reports/index.jsx — DV Finance
...
export default function Reports() {
  ...
  const currencyCode =
    moneySettings?.default_currency_code || moneySettings?.currency_code || 'INR';

  const fmt = (v) => moneyFormatter({ amount: Number(v ?? 0), currency_code: currencyCode });
  ...
  const buildAndDownloadCSV = (reportData, fromStr, toStr) => {
    ...
    const rows = [
      ['=== COLLECTION SUMMARY ==='],
      fromStr && toStr ? [`Date Range: ${fromStr} to ${toStr}`] : ['Date Range: All Time'],
      [],
      ['Metric', 'Amount'],
  ...
  const handleExportCSV = async () => {
    ...
    if (data) {
      ...
      try {
        ...
        message.success(
          from && to ? `Filtered report (${from} → ${to}) downloaded` : 'Full report downloaded'
        );
      } finally {
        setExporting(false);
      }
  ...
  const planColumns = [
    ...
    { title: 'Customers', dataIndex: 'customers', align: 'right' },
    {
      ...
      dataIndex: 'collected',
      align: 'right',
      render: (v) => <span style={{ color: '#16a34a' }}>{fmt(v)}</span>,
    },
    {
      title: 'Pending',
      dataIndex: 'pending',
      align: 'right',
      render: (v) => <span style={{ color: '#dc2626' }}>{fmt(v)}</span>,
    },
  ];
