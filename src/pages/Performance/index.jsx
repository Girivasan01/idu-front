/**
 * pages/Performance/index.jsx — DV Finance
...
function StaffCard({ staff, rank, moneyFormatter }) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 6, marginBottom: 14 }}
      >
        <Avatar
          size={42}
          icon={<UserOutlined />}
      ...
      <div style={{ marginTop: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
          ...
          <Typography.Text
            style={{ fontSize: 11, color: effColor(staff.efficiency), fontWeight: 600 }}
          >
            {staff.efficiency}%
          </Typography.Text>
        </div>
...
export default function Performance() {
  ...
  const { result: perfData, error } = useFetch(() => request.get({ entity: 'staff/performance' }));
  ...
  const columns = [
    ...
    },
    {
      ...
      align: 'right',
      render: (v) => (
        <span style={{ color: '#52c41a', fontWeight: 600 }}>{moneyFormatter({ amount: v })}</span>
      ),
    ...
    },
    {
      ...
      dataIndex: 'monthCollected',
      key: 'monthCollected',
      align: 'right',
      render: (v) => (
        <span style={{ color: BRAND, fontWeight: 500 }}>{moneyFormatter({ amount: v })}</span>
      ),
